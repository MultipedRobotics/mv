...
bool Serial::open(const std::string &port, int speed) {
  ...
  // port. fd = ::open(port.c_str(), O_RDWR|O_NOCTTY|O_NONBLOCK);
...
status: {'id': 1, 'error str': 'None', 'error num': 0, 'params': [253, 1],
'raw': [255, 255, 1, 4, 0, 253, 1, 252]}
*/
...
status_t Serial::decode() {
  ...
  for (int i = 0; i < buffer.size() - 2; ++i) {
    if (buffer[i] == 0xff && buffer[i + 1] == 0xff) {
      ...
      for (int j = 0; j < buffer[i + 3] - 2; ++j)
        ret.params.push_back(buffer[i + 5 + j]);
      break;
    }
  }
...
// Flush input buffer
void Serial::flush_input() { tcflush(fd, TCIFLUSH); }

// Flush output buffer
void Serial::flush_output() { tcflush(fd, TCOFLUSH); }

// Flush both input and output buffers
void Serial::flush() { tcflush(fd, TCIOFLUSH); }
